# PixelClock像素时钟(四)

  焊接和组装

## 目录

### 焊接前的信息

- 开窗
- 戴口罩 😷
- 电铬铁焊接的温度
  - 中温锡膏 195度
  - 锡线高温 245度
  - 压螺母 300度或350度

### 光敏元件(等待凝固时间长)

- 焊接光敏元件
- 粘到外壳上

### 按钮

- 锡丝 (中温 高温锡膏也行)
- 3个12mm 高头按钮
  - 只接按钮端的2根线
  - 拧下螺母
  - 装到后盖上, 拧上螺母
- 1个16mm 圆形带灯平台按钮
  - 只接按钮端的4根线
  - 拧下螺母
  - 装到外壳上, 拧上螺母

### 焊接ESP32C3针脚

- 针脚放到主板PCB上
- EPS32 C3放到针脚上
- 锡膏焊接(或先加助焊剂再加锡来焊接)
- 写入测试程序(请参考第三章)

### 压入螺母

- 外壳放在压螺母支架上
  - 支架
- 拧好M1.6螺丝和M1.6铜螺母
- 烙铁300度或350度
- 保持螺丝最长端刚刚好在螺母内
- 扣好PCB到后盖, 依次压入4个螺母
- 扣好PCB到放本来放置位置, 依次压次4个螺母(也可以只装对角的2个螺母)

### 焊接灯珠

- 中温锡膏(或高温锡膏)
- 两种方式
  - 手剂锡膏到灯板PCB上
  - 钢网
- 放置灯珠
  - 左上角有1 与 灯珠的缺角也保持左上角
  - ![灯珠方向](../Imags/灯珠摆放位置.png)
- 放到加热台
  - 完全熔化即可拿下来
- 测试灯板
  - 使用杜邦线(公对母 15CM长度)
  - 连接ESP32-C3 GND 5V I006
  - 灯板 GDN VCC IN
- 如果灯珠不亮
  - 检查不亮的灯和不亮灯前面一个灯是否连锡或虚焊
- 连接灯板再次测试
  - 连接灯板
  - 连接灯板和ESP32-C3

### 主板PCB

- 注意
  - PCB板上焊接, 推荐中温锡膏
  - 原因: 焊坏PCB板子也容易拆下来元件
- Type-C母座
  - 中温锡膏 (低温都可), 这里推荐用锡膏
  - 先预热PCB板子(重要, 放加热台上,再拿下来)
    - 立即涂上PCB板子Type-C处, 锡膏自动扩散薄薄一层
    - 如果多了, 擦掉重新预热再涂
    - 最边上固定Type-C母座2个孔洞也要涂上, 这里可以多涂一点
  - 再放上Typc-C母座, 放到加热台上, 熔化锡膏即可
- 测试 Type-C母座
  - 方式一
    - 主板PCB连接16mm平头按钮, 并按下按钮
      - 连接点PCB左 下 中 右对应平头按钮4根线
      - ![电源接线](../Imags/16mm电源按钮接线.jpg)
    - 通电, 插入Type-C 母座
    - 连接按钮的2个触点，如果灯亮就是Type-母座焊接好了, 否则重新焊接Type-C母座
  - 方式二
    - 万用表可以测相邻引脚无短路, 且与PCB有连接
- ESP32-C3
  - 长的金属针插入PCB
  - PCB与ESP-C3有线引出的焊接
  - GND引脚必须焊接
- 5.1k电阻
  - 2个贴片电阻加锡膏
  - 放入加热台(用铬铁也可)
- 10k电阻
  - 1个贴片电阻加锡膏
  - 电铬铁焊接
- 无源蜂鸣器
  - +(正)号在左
  - 与PCB焊接
- 声音传感器
  - 先连接针脚
    - 短的金属针在传感器 同一侧
    - 焊接 短金属针与传感器
  - 长的金属针再插入PCB
  - 与PCB焊接

### 连接按钮 LED线

- 连接主板与按钮线
  - 外壳有螺丝面朝上且16mm按钮在左, 无源蜂鸣器在上方
  - ![12mm按钮与主板接线位置.jpg](../Imags/12mm按钮与主板接线位置.jpg)
  - 俯视时, 从左到右, 按钮3 按钮2 按钮1,  对应后盖上按钮中 下 上
  - 2个注意点
    - 焊接注意连接线是偏向主板侧
    - 使用中温锡膏焊接
- 连接主板和电源灯的线
  - 左下中右的方式连接
- 连接主板与LED灯
  - LED灯一面朝下, 且有VCC GND IN 在右
  - 2个注意点
    - 焊接注意线是偏向主板侧
    - 使用中温锡膏焊接
- 连接光敏元件
  - 暂时这样, 绕的方式

### 测试连线正常

- 插入ESP32-C3上的Type-C供电
- 灯正常亮说明接线正确
- 然后刷入固件
- 按下ESP32-C3上的2个按钮会重启
- 启动会显示START
- 随后显示PIX10.0.0.1
  - 表示正常

### 连接亚力克板 均光膜 LED

- 撕下美纹胶 缠绕屏幕周围的外壳, 防止706粘到外壳上不好清理
- 保持光栅和均光膜干净
- LED扣到光栅上
- 放上均光膜
- 亚力克板只撕一面, 撕掉的一面贴合LED
- 均匀的涂上706
- 拿上小工具, 重物压着 2小时

### 拧螺丝

- 拧上主PCB的螺丝
- 宁上后盖上的螺丝

### 最后

- 插电
- 撕掉亚力克板的膜
- 连接PIX的WIFI
- 访问10.0.0.1
- 填入家中的网络名和密码
- 当当当! 组装完成了! ✅
